...
# Guards the apple to the end of game
class Bot(IBot):
    ...
    def chooseDirection(self, snake: Snake, opponent: Snake, mazeSize: Coordinate, apple: Coordinate) -> Direction:
        ...
        for d in directions:
            ...
            if new_head.inBounds(mazeSize) \
                ...
                if concedeApple:
                    ...
                    value += (1 - new_head.getMathDistance(apple) / mazeSize.x)  # prefer cells closer to apple
                direction_tuple = (d, new_head, value)
        ...
        if winning_directions:
            result = max(winning_directions, key=lambda d: d[2])[0]
        elif safe_directions:
            if len(safe_directions) > 1 and len(snake.body) > 5 \
                    and len(snake.body) > len(opponent.body) \
                    and 0 < apple.x < mazeSize.x - 1 and 0 < apple.y < mazeSize.y - 1 \
                    and snake.head.getDistance(apple) == 1 and opponent.head.getDistance(apple) > 1:
                avoid_apple_directions = [d for d in safe_directions if d[1] != apple]
                return min(avoid_apple_directions, key=lambda d: d[1].getMathDistance(apple))[0]

            result = max(safe_directions, key=lambda d: d[2])[0]
        elif risky_directions:
            result = max(risky_directions, key=lambda d: d[2])[0]
        elif possible_directions:
            result = max(possible_directions, key=lambda d: d[2])[0]
        else:
